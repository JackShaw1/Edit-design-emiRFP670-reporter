...
original_seq = 'TCATCATCAGGTATTTCC'.upper()
...
for seq in all_possible_seqs:
    if len(seq) > 2:
        if len(seq) == 6 and seq[3:] == 'CAT' or seq[:3] == 'CAT':
            
            total_seq = reverse_complement(seq) + original_seq
            RFP670_seq = translate_from_start(total_seq)
            gc_content = calc_gc_content(seq)
            length = len(seq)

            if len(RFP670_seq) > 3:
                with open('possible_template_insertions.csv', 'a', newline='') as outfile:
